...
jQuery_T4NT(document).ready(function($) {
  // FIXED not show preview on shopify, only run on demo kalles
  // typeof(Shopify.PreviewBarInjector) == "undefined"
  if (location.hostname.indexOf(ck_hostname) > -1 && !designMode) {
  
    $.ajax({
        dataType: "html",
        type: 'GET',
        url: 'https://' + ck_hostname + '.myshopify.com/preview_bar/?preview_theme_id='+themeIDt4,
        success: function(data) {
          console.log('pv_bar done');
          //location.reload(url);
        },
        error: function (data) {
          console.log('pv_bar error');
        },
        complete: function () {
          //location.reload(themeurl);
        },
      });
  }
  // END FIXED not show preview on shopify
